...
| 部分应援棒镜像写 | **FE00** 下 **FF11** | 与 FF01 并存时可能需**双写**同一条 payload |
...
| **无出厂校准** | 消费级应援棒通常没有显示器式的色准流程，官方征极类 App 也可能偏色。 |
...
1. **总开关**  
   - **`BleController.autoSyncEnabled`**：关闭则播放时不向应援棒发任何「跟歌」变色。
...
2. **两种同步方式**（灯光同步打开后，由 **`MusicController.useMetronomeBeatSync`** 区分，**默认关闭节拍器 = 时间轴模式**）  
   ...
   | **节拍器** | 按 **BPM**（默认 120，可调 60～200）与播放进度做**等相位间隔**变色；每次拍点用 **HSV 色相递增** 调 `updateLightColor`。不依赖波形/原生解码插件，便于快速构建。 |

3. **性能与策略**  
   - 节拍器为纯 Dart 相位逻辑，无整文件波形分析。  
   - **BPM 滑条**在「灯光同步 + 节拍器」同时开启时可用。

4. **说明**  
   - 节拍与真实鼓点是否一致取决于你设的 BPM；复杂编曲可手动微调 BPM。
